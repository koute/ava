...
The image of `games/tcc-opt/Dockerfile` copies tinycc into `/opt/scaffold` from `games/tcc-opt/tinycc.tar.gz`, the tree of the pinned commit vendored as an archive under the LGPL it is distributed under, since repo.or.cz serves a commit by hash only while it heads a branch. It builds the bootstrap from it with gcc, fetches the pinned suites by checksum, records the deflate fixture and installs binutils, the libc headers, make and tcl over the base.
